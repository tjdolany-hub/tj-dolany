export default function Loading() {
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="text-center mb-8">
        <div className="h-4 w-16 bg-surface-muted rounded mx-auto mb-2" />
        <div className="h-10 w-56 bg-surface-muted rounded mx-auto" />
      </div>
      <div className="h-12 w-full max-w-md bg-surface-muted rounded-lg mx-auto mb-12" />
      <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
        {Array.from({ length: 12 }).map((_, i) => (
          <div key={i} className="bg-surface rounded-xl border border-border overflow-hidden animate-pulse">
            <div className="aspect-square bg-surface-muted" />
            <div className="p-3 space-y-2">
              <div className="h-4 w-full bg-surface-muted rounded" />
              <div className="h-3 w-16 bg-surface-muted rounded mx-auto" />
            </div>
          </div>
        ))}
      </div>
    </div>
  );
}
